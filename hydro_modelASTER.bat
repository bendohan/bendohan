...
saga_cmd ta_hydrology 0 -ELEVATION=%od%\%pre%sinks_removed.sgrd -SINKROUTE=%od%\%pre%sink_route.sgrd -WEIGHTS=NULL -FLOW=%od%\%pre%flow.sgrd -VAL_INPUT=NULL -ACCU_MATERIAL=NULL -STEP=1 -FLOW_UNIT=0 -FLOW_LENGTH=NULL -LINEAR_VAL=NULL -LINEAR_DIR=NULL -METHOD=4 -LINEAR_DO=1 -LINEAR_MIN=500 -CONVERGENCE=1.1

::Run Channel Network
saga_cmd ta_channels 0 -ELEVATION=%od%\%pre%sinks_removed.sgrd -SINKROUTE=NULL -CHNLNTWRK=%od%\%pre%channel_network.sgrd -CHNLROUTE=%od%\%pre%channel_route.sgrd -SHAPES=%od%\%pre%channel_vector.shp -INIT_GRID=%od%\%pre%flow.sgrd -INIT_METHOD=2 -INIT_VALUE=1000 -DIV_GRID=NULL -DIV_CELLS=5 -TRACE_WEIGHT=NULL -MINLEN=10

...
To upload data from OpenStreetMap in my PostGIS database, I used a program called osm2pgsql, which can be accessed [on github](https://github.com/openstreetmap/osm2pgsql). We used a [batch script](convertOSM.bat) written by our Professor and the default [.style file](dsm.style) in the upload. We didn't alter the style file at all, but if wanted tags that were not included in the base file we could have added those, and we could have removed tags as well, although we didn't because we weren't sure what direction we wanted to take the project when we uploaded the data. We only ended up using, the width, highway, building, and amenity tags, so could have removed them from the style file to clean up the data and make it a little faster.
...
I made a [leaflet map](dsmap/index.html) showing the percent of houses in each subward accessible by road, with the healthcare options overlaid. For this map I used the QGIS plugin qgis2web. It did require reprojecting my project from EPSG 4326 to EPSG 3857, the projection of the OpenStreetMap basemap.
